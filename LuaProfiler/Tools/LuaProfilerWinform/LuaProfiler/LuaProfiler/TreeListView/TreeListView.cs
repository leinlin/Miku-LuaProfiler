...
namespace System.Windows.Forms
{
    ...
    public class TreeListView : System.Windows.Forms.ListView
    {
        ...
        #region 事件, 委托以及 内部调用（Internal calls）
        #region 事件定义
        /// <summary>
        /// 当用户编辑item的标签后触发。
        /// Occurs when the label for an item is edited by the user.
        /// </summary>
        [Description("当用户编辑item的标签后触发。\r\nOccurs when the label for an item is edited by the user.")]
        ...
        /// </summary>
        [Description("当用户开始编辑item的label前触发。\r\nOccurs when the user starts editing the label of an item."), Browsable(true)]
        ...
        {
            //MessageBox.Show("编辑之后的值：" + e.Label);
            //MessageBox.Show("编辑之前的值：" + FocusedItem.SubItems[_lastitemclicked.ColumnIndex].Text);
            TreeListViewBeforeLabelEditEventArgs labelEditEventArgs = new TreeListViewBeforeLabelEditEventArgs(
                item: FocusedItem,
                column: _lastitemclicked.ColumnIndex, 
                label: FocusedItem.SubItems[_lastitemclicked.ColumnIndex].Text,
               oldLabel_: e.Label
               );
        ...
        {
            //https://msdn.microsoft.com/zh-cn/system.windows.forms.treeview.onbeforelabeledit.aspx
            //在派生类中重写 OnBeforeLabelEdit 时，一定要调用基类的 OnBeforeLabelEdit 方法，以便已注册的委托对事件进行接收


            //MessageBox.Show("OnBeforeLabelEdit");
            TreeListViewBeforeLabelEditEventArgs labelEditEventArgs = new TreeListViewBeforeLabelEditEventArgs(FocusedItem, _lastitemclicked.ColumnIndex, FocusedItem.SubItems[_lastitemclicked.ColumnIndex].Text);
        ...
        #endregion
        #region FocusedItem 焦点Item
        /// <summary>
        /// 获取当前焦点所在的项。 Gets the item in the control that currently has focus.
        /// </summary>
        [Browsable(false)]
            ...
            switch ((APIsEnums.WindowMessages)m.Msg)
            {
                ...
                case (APIsEnums.WindowMessages)APIsEnums.ReflectedMessages.NOTIFY:
                    ...
                    switch ((APIsEnums.ListViewNotifications)nmhdr.code)
                    {
                        ...
                        case APIsEnums.ListViewNotifications.BEGINLABELEDIT:
                            ...
                            while (item.SubItems.Count - 1 < _lastitemclicked.ColumnIndex)
                                item.SubItems.Add("");

                            TreeListViewBeforeLabelEditEventArgs beforeed = new TreeListViewBeforeLabelEditEventArgs(FocusedItem, _lastitemclicked.ColumnIndex, item.SubItems[_lastitemclicked.ColumnIndex].Text);
                ...
                case APIsEnums.WindowMessages.LBUTTONDOWN:
                    ...
                    if (item.GetBounds(TreeListViewItemBoundsPortion.PlusMinus).Contains(PointToClient(MousePosition)) &&
                        ShowPlusMinus && !(item.Items.Count == 0 && item.HasChildren ) &&
    ...
    /// <summary>
    /// TreeListViewLabelEdit编辑事件的参数。Arguments of a TreeListViewLabelEdit event.
    /// </summary>
    [Serializable]
    public class TreeListViewLabelEditEventArgs : CancelEventArgs
    {
        ...
        #region Properties
        /// <summary>
        /// 获取子项的Label值。Gets the label of the subitem
        /// </summary>
        ...
        #region Constructor
        /// <summary>
        /// 创建TreeListViewLabelEditEventArgs实例
        /// </summary>
        /// <param name="item">当前操作的TreeListViewItem</param>
        /// <param name="column">当前列索引</param>
        /// <param name="label">当前Label值</param>
        public TreeListViewLabelEditEventArgs(TreeListViewItem item, int column, string label) : base()
        {
            _item = item; _colindex = column; Label = label;
        }
        ...
        /// <param name="column">当前列索引</param>
        /// <param name="label">当前Label值</param>
        /// <param name="labelOld_">编辑之前的Label值</param>
        public TreeListViewLabelEditEventArgs(TreeListViewItem item, int column, string label, string labelOld_) : base()
        {
            //MessageBox.Show("基类，当前值：" + labelOld_ + ",原始值：" + label);
            _item = item;
            _colindex = column;
            Label = labelOld_;
            LabelOld = label;
    ...
    public class TreeListViewBeforeLabelEditEventArgs : TreeListViewLabelEditEventArgs
    {
        ...
        #region Constructor
        /// <summary>
        /// 创建 TreeListViewBeforeLabelEditEventArgs 实例
        /// </summary>
        /// <param name="item">当前TreeListViewItem</param>
        /// <param name="column">当前列索引</param>
        /// <param name="label">当前Label值</param>
        ...
        /// <param name="column">当前列索引</param>
        /// <param name="label">当前Label值</param>
        /// <param name="oldLabel_">编辑之前的Label值</param>
        public TreeListViewBeforeLabelEditEventArgs(TreeListViewItem item, int column, string label, string oldLabel_) : base(item, column, label, oldLabel_) { }
